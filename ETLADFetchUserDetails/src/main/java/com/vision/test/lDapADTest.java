/*javac -cp ".*:" lDapADTest.java
java -cp ".*:" lDapADTest


compile - javac -cp ".:*.jar" lDapADTest.java
run - java -cp ".:*.jar" lDapADTest*/
...
//				retryCount++;
			...
			/*	 System.out.println("Authenticating -> " + username + "@" + domainName + " through "
				...
				if (currentLdapUrlIndex < ldapServerUrls.length) {
					currentLdapUrlIndex++;
					/*
					 * if (currentLdapUrlIndex == ldapServerUrls.length) { currentLdapUrlIndex = 0;
					 * }
					 */
					continue;
				}
